...
const { parseProps, parseNpmName, generateEntry, asyncDebounce, camel2KebabComponentName, kebab2CamelComponentName } =
  UTILS;
...
function getUsedComponentMetas(rootDir, lowcodeDir = "lowcode", metaFilename, components) {
  let metaPaths = glob.sync(path.resolve(rootDir, `${lowcodeDir}/**/${metaFilename}.@(js|ts|jsx|tsx)`));
  if (metaPaths && metaPaths.length) {
    metaPaths = metaPaths.map((item) => {
      return item.slice(path.resolve(rootDir, lowcodeDir).length + 1, item.lastIndexOf(metaFilename) - 1);
    });
  }
  return components
    ? components.filter((component) => {
...
function confirmMetaTypes(rootDir, lowcodeDir, metaTypes) {
  return metaTypes.filter((item) => {
    ...
    const res = glob.sync(path.resolve(rootDir, `${lowcodeDir}/**/${metaFilename}.@(js|ts|jsx|tsx)`));
    return res.length;
  });
}
...
async function build(options, pluginOptions, execCompile) {
  ...
  const result = {
    ...
    assetsPaths: await bundleAssets(options, pluginOptions, confirmedMetaTypes, confirmedRenderPlatforms, execCompile),
  };
  if (bundleEachComponentMeta) {
    result.componentMetaPath = await bundleComponentMeta(webPackConfig, options, pluginOptions, execCompile);
  }
  return result;
}
...
async function start(options, pluginOptions) {
  ...
  if (baseLibrary === "rax" && Array.isArray(extraAssets)) {
    extraAssets.push("/downloadAssets/code/npm/@alife/mobile-page/0.1.1/build/lowcode/assets-prod.json");
  }
  const metaExportName = `${PARSED_NPM_NAME.uniqueName}Meta`;
  const { viewPath, metaPathMap, renderViewPathMap, platforms } = await debounceBuild(options, pluginOptions, false);
...
async function initLowCodeSchema(
  ...
  if (lowcodeDirExists) {
    ...
    const componentsDirs = await fse.readdir(path.resolve(rootDir, componentsPath || "src/components"));
    const lowcodeDirMap = {};
    if (!lowcodeDirs) {
      return;
    }
  ...
  filteredComponents.forEach((item) => {
    ...
    if (
      !fse.existsSync(
        path.resolve(rootDir, `${lowcodeDir}/${componentNameFolder}/meta${metaDevSubfix}.${metaFormat || "ts"}`)
      ) &&
      !fse.existsSync(path.resolve(rootDir, `${lowcodeDir}/${componentNameFolder}/meta${metaDevSubfix}.js`))
    ) {
...
const ${item.componentName}Meta: ComponentMetadata = ${JSON.stringify(componentDescription, null, 2)};
...
async function bundleMetaV2(options, pluginOptions, execCompile, metaType) {
  ...
  if (fullbackMeta) {
    ...
    fullbackComponents = getUsedComponentMetas(rootDir, lowcodeDir, `meta${fullbackMetaSuffix}`, components);
  }
  const usedComponents = getUsedComponentMetas(rootDir, lowcodeDir, `meta${metaSuffix}`, components);
  const componentsImportStr = fullbackComponents
    .map((component) => {
      const componentNameFolder = camel2KebabComponentName(component);
      let metaJsPath = path.resolve(rootDir, `${lowcodeDir}/${componentNameFolder}/${metaFilename}`);
      if (!usedComponents.includes(component) && fullbackComponents.includes(component)) {
        metaJsPath = path.resolve(rootDir, `${lowcodeDir}/${componentNameFolder}/meta${fullbackMetaSuffix}`);
        usedComponents.push(component);
      }
      metaJsPath = metaJsPath.replace(/\\/g, "\\\\");
      return `import ${component.includes(".") ? component.replace(/\./g, "") : component}Meta from '${metaJsPath}'`;
    })
    .join("\n");
...
async function bundleEditorView(webPackConfig, options, pluginOptions, metaPathMap, platforms, execCompile) {
  const { registerTask, getAllTask, onGetWebpackConfig } = options;
  ...
  const advancedRenderUrls = {};
  platforms.forEach((platform) => {
    advancedRenderUrls[platform] = [`./render/${platform}/view.js`, `./render/${platform}/view.css`];
  });
  let _setterMap = "{";
  const setterImportStr = Object.keys(setterMap || {})
    .map((item) => {
  ...
  const indexJsParams = {
    ...
    library,
    urls: JSON.stringify(
      renderUrls || umdUrls || [`${buildTarget}/${lowcodeDir}/view.js`, `${buildTarget}/${lowcodeDir}/view.css`]
    ),
    editUrls: JSON.stringify(
      editUrls || umdUrls || [`${buildTarget}/${lowcodeDir}/view.js`, `${buildTarget}/${lowcodeDir}/view.css`]
    ),
    advancedRenderUrls: JSON.stringify(advancedRenderUrls),
  ...
  if (!execCompile || editUrls || umdUrls || getAllTask().includes("lowcode-editor-view")) return viewPath;
...
async function bundleRenderView(options, pluginOptions, platform, execCompile) {
  ...
  let componentViewsExportStr;
  let componentViewsImportStr;
  const _componentViews = getUsedComponentViews(rootDir, `src/${platform}/components`, components) || [];
  ...
  componentViewsImportStr = _componentViews
    .map((component) => {
      const componentNameFolder = camel2KebabComponentName(component);
      const viewJsPath = path.resolve(rootDir, `src/${platform}/components/${componentNameFolder}/view`);
      return `import * as ${component}Data from '${viewJsPath}'`;
    })
    .join("\n");
  const scssEntry = getScssEntry(rootDir);
  ...
  if (!execCompile || editUrls || umdUrls || getAllTask().includes(`render-view-${platform}`)) return viewPath;
...
async function bundleAssets(options, pluginOptions, metaTypes, renderTypes, execCompile) {
  ...
  if (baseLibrary === "rax" && Array.isArray(extraAssets)) {
    extraAssets.push(`https://g.alicdn.com/code/npm/@alife/mobile-page/0.1.1/build/lowcode/assets-prod.json`);
  }
    ...
    builtinAssets.map(async (url) => {
      if (typeof url === "object") {
        ...
        try {
          ...
          console.error(`[@alifd/build-plugin-lowcode] get assets data from builtin assets ${url} failed: `, e);
          return {};
        }
      }
    })
  ...
  const assetsPaths = await Promise.all(
    ["daily", "prod", "dev"].map(async (item) => {
      const _baseUrl =
        (baseUrl && baseUrl[item]) || `${UNPKG_BASE_URL_MAP[engineScope]}/${package.name}@${package.version}`;
      let urls;
      let metaUrl;
      const metaUrls = {};
      const advancedRenderUrls = {};
      ...
      if (item === "dev") {
        ...
        renderTypes.forEach((renderType) => {
          advancedRenderUrls[renderType] = [`./render/${renderType}/view.js`, `./render/${renderType}/view.css`];
        });
      } else {
        urls = JSON.stringify([
  ...
  onHook("after.build.compile", () => {
    ...
    /** 输出静态资源到默认配置文件夹 */
    if (pluginOptions && pluginOptions.generateDownloadAssets) {
      const destDir = path.join(path.resolve(rootDir, `${buildTarget}/${lowcodeDir}`), "downloadAssets");
      const sourceDir = path.join(__dirname, "public", "downloadAssets");
      try {
        fse.copySync(sourceDir, destDir, { overwrite: true });
        console.log(`${sourceDir} copy to ${destDir}  success!`);
      } catch (err) {
        console.error(err);
      }
    }
...
function updatePackage(rootDir, baseUrl, lowcodeDir = "lowcode", buildTarget = "build", engineScope = "@ali") {
...
async function bundleComponentMeta(webPackConfig, options, pluginOptions, execCompile) {
  ...
  let { components } = pluginOptions || {};
  const { devAlias, externals = {}, buildTarget = "build", lowcodeDir = "lowcode" } = pluginOptions || {};
  if (components && !Array.isArray(components)) {
    console.error("[@alifd/build-plugin-lowcode] components must be Array<ComponentName: string>");
    components = null;
  }
  ...
  onHook("after.build.compile", () => {
    usedDevComponents.forEach((comp) => {
      const componentNameFolder = camel2KebabComponentName(comp);
      const componentJsPath = `${lowcodeDir}/${componentNameFolder}/${metaFilename}`;
      const originPath = path.resolve(rootDir, `${buildTarget}/${lowcodeDir}/${componentJsPath}.js`);
      ...
      try {
        const targetPath = path.resolve(rootDir, `${buildTarget}/${lowcodeDir}/${componentJsPath}.json`);
        fse.outputFileSync(targetPath, JSON.stringify(require(originPath), null, 2));
      } catch (e) {}
    });
  });
